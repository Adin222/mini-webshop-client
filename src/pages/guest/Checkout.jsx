...
//Paths
import { getCartItemsPath, createOrderPath } from "../../paths/paths";
  ...
  const handleSubmitOrder = async (e) => {
    ...
    if (!phoneRegex.test(phone)) {
      setToastMessage({
        status: "danger",
        open: true,
        message: "Phone number can only have numbers",
      });
      return;
    }
    ...
    setLoading(true);
    try {
      const response = await Post(createOrderPath, body);
        ...
        <div className="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
              ...
              <div className="sm:col-span-2">
                <label htmlFor="address" className="block text-sm font-medium">
                  Address <span className="text-gray-400">(required)</span>
                </label>
                <input
                  type="text"
                  id="address"
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-4 mb-8 w-full rounded-md bg-indigo-700 px-6 py-3 font-medium text-white hover:cursor-pointer hover:bg-indigo-500 disabled:cursor-not-allowed disabled:bg-indigo-400"
            >
              Place Order
            </button>
          </form>
